...
function checkInputR(form)
{
    ...
    err += checkFIO(form.userFIO.value);
    err += checkOTCH(form.userOTCH.value);
    err += checkAGE(form.userAGE.value);
...
function checkOTCH(field)
{
    if (field == "")
        return "Не введено отчество.\n";

function checkAGE(field)
{
    if (field == "")
        return "Не введена дата рождения.\n";
}
...
function ConfirmDel(fname){
    if(confirm('Удалить файл "'+fname+'"?')) return true;
    else return false;
}